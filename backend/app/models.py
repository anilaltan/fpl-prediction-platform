...
class Player(Base):
    ...
    entity_mapping = relationship(
        "EntityMapping", back_populates="player", uselist=False
    )

    # Constraints
    __table_args__ = (
        CheckConstraint(
            "position IN ('GK', 'DEF', 'MID', 'FWD')", name="check_position"
        ),
    )
...
class PlayerStats(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "gameweek",
            "season",
            name="uq_player_stats_player_gameweek_season",
        ),
    )
...
class TeamStats(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "team_id", "gameweek", "season", name="uq_team_stats_team_gameweek_season"
        ),
    )
...
class Prediction(Base):
    ...
    )  # Use fpl_id for consistency with PlayerGameweekStats
    gameweek = Column(Integer, nullable=False, index=True)
    ...
    )  # expected_points (renamed from predicted_points for clarity)
    ...
    __table_args__ = (
        UniqueConstraint(
            "fpl_id", "gameweek", "season", name="uq_prediction_fpl_gameweek_season"
        ),
    )
...
class BacktestResult(Base):
    ...
    )  # 'expanding_window' or 'rolling_window'
...
class PlayerGameweekStats(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "fpl_id", "gameweek", "season", name="uq_player_gameweek_season"
        ),
    )
...
class MarketIntelligence(Base):
    ...
    )  # Rank by expected points (descending: highest xP = rank 1)
    ownership_rank = Column(
        Integer, nullable=False
    )  # Rank by ownership percentage (descending: highest ownership = rank 1)
