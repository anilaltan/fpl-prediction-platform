...
from app.schemas import (
    FormAlphaOptimizeRequest,
    FormAlphaResponse,
    FDRFitRequest,
    FDRResponse,
    DefConFeaturesResponse,
    StochasticFDRRequest,
    StochasticFDRResponse,
    FDRComparisonResponse,
    FDRVerificationResponse,
    XMinsPredictionRequest,
    XMinsPredictionResponse,
    ...
    OwnershipArbitrageRequest,
    OwnershipArbitrageResponse,
    CaptainSelectionRequest,
    CaptainSelectionResponse,
    ChipAnalysisRequest,
    ChipAnalysisResponse,
    ComprehensiveRiskAnalysisRequest,
    ...
    BulkCleaningRequest,
    BulkCleaningResponse,
    DefConMetricsResponse,
    MarketIntelligenceResponse,
    MarketIntelligencePlayer,
    TeamPlanRequest,
    TeamPlanResponse,
    TransferStrategy,
)
...
from app.services.third_party_data import (
...
def _update_cache(gameweek: int, players_data: List, dream_team=None):
    ...
    logger.info(
        f"Cache updated for GW{gameweek} with {len(players_data)} players at {datetime.now()}"
    )
...
async def health_check():
    ...
    if current_gw in DATA_CACHE["last_updated_by_gw"]:
        cache_age = (
            datetime.now().timestamp() - DATA_CACHE["last_updated_by_gw"][current_gw]
        )
...
async def get_cache_status():
    ...
    return {
        ...
        "last_updated": datetime.fromtimestamp(
            DATA_CACHE["last_updated_by_gw"][current_gw]
        ).isoformat()
        if current_gw in DATA_CACHE["last_updated_by_gw"]
        else None,
    }
...
async def update_predictions_endpoint(
    gameweek: Optional[int] = None, background_tasks: BackgroundTasks = None
):
    """
    ...
    """
    try:
        from app.scripts.update_predictions import (
            update_predictions_for_gameweek,
            update_predictions_for_current_gameweek,
        )
...
async def optimize_form_alpha(
    ...
    try:
        ...
        if "points" not in df.columns:
            raise HTTPException(
                status_code=400, detail="Historical data must include 'points' column"
            )
        ...
        if store_result:
            try:
                ...
                current_gw = (
                    ...
                    if hasattr(fpl_api, "get_current_gameweek")
                    else 1
                )

                # Check if entry exists for this gameweek
                existing = (
                    db.query(FormAlpha).filter(FormAlpha.gameweek == current_gw).first()
                )
    ...
    try:
        ...
        for team_name in feature_service.fdr_model.attack_strengths.keys():
            fdr_data.append(
                FDRResponse(
                    team_name=team_name,
                    attack_strength=feature_service.fdr_model.attack_strengths[
                        team_name
                    ],
                    defense_strength=feature_service.fdr_model.defense_strengths.get(
                        team_name, 0.0
                    ),
...
async def get_defcon_features(player_id: int, position: str = "MID", minutes: int = 90):
    """
...
async def predict_xmins(request: XMinsPredictionRequest):
    ...
    try:
        ...
        p_start = ml_engine.xmins_model.predict_start_probability(
            request.player_data, request.fixture_data
        )
        ...
        return XMinsPredictionResponse(
            p_start=p_start, expected_minutes=expected_minutes
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def predict_attack(request: AttackPredictionRequest):
    ...
    try:
        ...
        if request.fixture_data:
            ...
            if opponent_team_id:
                ...
                opponent_data = {
                    ...
                    "defense_strength": request.fdr_data.get(
        ...
        predictions = ml_engine.attack_model.predict(
            request.player_data, request.fixture_data, request.fdr_data, opponent_data
        )

        return AttackPredictionResponse(xg=predictions["xg"], xa=predictions["xa"])
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        ...
        if xcs > 0:
            ...
            expected_goals_conceded = (
                1.5 * (1.0 / max(0.1, team_defense)) * opponent_attack * home_factor
            )
...
async def predict_momentum(request: MomentumPredictionRequest):
    ...
    try:
        ...
        return MomentumPredictionResponse(
            momentum=momentum, trend=trend, forecast=forecast
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
@app.post(
    "/api/predictive/comprehensive", response_model=ComprehensivePredictionResponse
)
    ...
    try:
        ...
        if len(historical_points) >= 2:
            ...
            if len(historical_points) >= 6:
                ...
                previous = (
                    historical_points[-1] if len(historical_points) > 1 else recent
                )
            trend = float(recent - previous)
            momentum = trend
        else:
    ...
    try:
        ...
        for player in request.players:
            ...
            for week in range(2, request.horizon_weeks + 1):
                attr_name = f"expected_points_gw{week}"
                if (
                    hasattr(player, attr_name)
                    and getattr(player, attr_name) is not None
                ):
        ...
        for week, transfer_info in solution.get("transfers", {}).items():
            transfer_strategy.append(
                TransferStrategy(
                    ...
                    expected_points_gain=solution.get("points_breakdown", {})
                    .get(week, {})
                    .get("expected_points", 0.0),
                )
            )
...
async def get_market_intelligence(
    ...
    try:
        ...
        df = market_intelligence_service.calculate_player_ranks(
            db=db, gameweek=gameweek, season=season, use_fpl_api_ownership=True
        )
        ...
        # Count categories
        differentials_count = sum(
            1 for p in market_players if p.category == "Differential"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Market intelligence error: {str(e)}"
        )
...
async def comprehensive_risk_analysis(request: ComprehensiveRiskAnalysisRequest):
    ...
    try:
        ...
        analysis = risk_service.get_comprehensive_analysis(
            players_data, current_squad_data, request.gameweek, optimized_squad_data
        )
...
async def run_backtest(request: BacktestRequest):
    ...
    try:
        ...
        def prediction_function(train_data, test_data):
            ...
            mean_points = (
                train_data["points"].mean() if "points" in train_data.columns else 0.0
            )
        ...
        if request.methodology == "expanding_window":
            result = backtesting_engine.expanding_window_backtest(
                df, prediction_function, min_train_weeks=request.min_train_weeks
            )
        else:
            window_size = request.window_size or request.min_train_weeks
            result = backtesting_engine.rolling_window_backtest(
                df, prediction_function, window_size=window_size
            )
...
async def get_model_performance(season: str = "2025-26", db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        summaries = (
            db.query(BacktestSummary).filter(BacktestSummary.season == season).all()
        )

        # Get backtest results (detailed per-gameweek)
        results = (
            db.query(BacktestResult)
            .filter(BacktestResult.season == season)
            .order_by(BacktestResult.gameweek)
            .all()
        )

        # Get model performance records
        model_perf = (
            db.query(ModelPerformance).order_by(ModelPerformance.gameweek).all()
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching model performance: {str(e)}"
        )
...
async def get_all_players(
    ...
    try:
        ...
        if gameweek is None:
            try:
                if use_next_gameweek:
                    ...
                    logger.info(
                        f"[BATCH] Using next gameweek for Dashboard: {gameweek}"
                    )
                else:
        ...
        if gameweek is None or gameweek < 1:
            gameweek = 1
            logger.warning("[BATCH] Invalid gameweek, using default: 1")
        ...
        try:
            ...
            if cached_players is not None:
                logger.info(
                    f"[CACHE HIT] Returning {len(cached_players)} cached players for GW{gameweek}"
                )
                if limit:
            ...
            logger.warning(
                f"[CACHE] Cache lookup failed: {str(cache_error)}, continuing..."
            )
    ...
    try:
        ...
        predictions = (
            db.query(Prediction)
            .filter(Prediction.gameweek == gameweek, Prediction.season == "2025-26")
            .all()
        )

        logger.info(
            f"[BATCH] Loaded {len(predictions)} predictions from database for GW{gameweek}"
        )
        ...
        players = (
            db.query(Player).options(joinedload(Player.team)).limit(max_players).all()
        )
        ...
        try:
            ...
            bootstrap = await asyncio.wait_for(
                fpl_api.get_bootstrap_data(), timeout=5.0
            )
            ...
            logger.info(
                f"[FPL API] Loaded ownership data for {len(ownership_map)} players"
            )
        except asyncio.TimeoutError:
            logger.warning(
                "[FPL API] Bootstrap data fetch timed out (>5s), skipping ownership data"
            )
        except Exception as e:
        ...
        try:
            ...
            max_gw_subq = (
                ...
                .group_by(PlayerGameweekStats.fpl_id)
                .subquery()
            )
            ...
            latest_stats = (
                ...
                .filter(PlayerGameweekStats.season == "2025-26")
                .all()
            )
            ...
            logger.info(
                f"[FORM] Loaded form data for {len(form_map)} players in batch (replaced {len(player_ids)} individual queries)"
            )
        except Exception as form_batch_error:
            logger.warning(
                f"[FORM] Batch form query failed: {str(form_batch_error)}, using 0.0 as default"
            )
            form_map = {}
        ...
        for player in players:
            try:
                ...
                try:
                    team_name = (
                        player.team.name
                        if player.team and hasattr(player.team, "name")
                        else "Unknown"
                    )
                except Exception:
                    team_name = "Unknown"
                ...
                players_data.append(
                    PlayerDisplayData(
                        ...
                        defcon_score=float(round(defcon_score, 2))
                        if defcon_score is not None
                        else 0.0,
                    )
                )
                ...
                logger.error(
                    f"[SKIP] Player {player.id} ({player.name}): {str(player_error)}"
                )
                continue
        ...
        try:
            ...
            if basic_data:
                logger.info(
                    f"[FALLBACK] Returning {len(basic_data)} basic player records"
                )
                return basic_data
...
async def _get_basic_player_data(
    db: Session, limit: Optional[int] = None
) -> List[PlayerDisplayData]:
    """
    ...
    try:
        ...
        for player in players:
            try:
                ...
                logger.error(
                    f"[BASIC] Failed to process player {getattr(player, 'id', 'unknown')}: {str(player_error)}"
                )
                continue
...
async def get_dream_team(gameweek: Optional[int] = None, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if cached_players is None:
            ...
            logger.info(
                f"[CACHE MISS] Triggering player data calculation for GW{gameweek}"
            )
        ...
        try:
            ...
            if solution.get("optimal", False):
                ...
                for player_id in squad_week1:
                    player_dict = next(
                        (p for p in players_data if p["id"] == player_id), None
                    )
                    if player_dict:
                        squad.append(
                            DreamTeamPlayer(
                                ...
                                team=player_dict.get(
                                    "team_name", "Unknown"
                                ),  # team should be string
                ...
                logger.info(
                    f"[SOLVER SUCCESS] Dream team optimized: {len(squad)} players"
                )
            else:
                logger.warning(
                    f"[SOLVER] Optimization not optimal: {solution.get('status', 'unknown')}"
                )
...
async def _get_fallback_dream_team(
    players_data: List[Dict], gameweek: int
) -> DreamTeamResponse:
    """
    ...
    try:
            ...
            try:
                squad.append(
                    DreamTeamPlayer(
                        ...
                        team=str(team_name)
                        if team_name
                        else "Unknown",  # team should be string (team name)
        ...
        logger.info(
            f"[FALLBACK SUCCESS] Squad: {len(squad)}, XI: {len(starting_xi)}, Formation: {formation}"
        )
...
async def get_fixtures_data(
    ...
    try:
        ...
        fixtures = await fpl_api.get_fixtures(
            gameweek=gameweek, future_only=future_only
        )
...
async def get_players_bulk_history(
    ...
    try:
        ...
        players = await fpl_api.get_all_players_with_history(
            player_ids=id_list, max_players=max_players
        )
...
async def get_understat_players(
    season: str = "2025", player_name: Optional[str] = None
):
    """
    ...
    try:
        ...
        return {"season": season, "players": players, "count": len(players)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/third-party/fbref/defensive")
async def get_fbref_defensive_stats(
    season: str = "2025-2026", player_name: Optional[str] = None
):
    """
    ...
    try:
        ...
        return {"season": season, "players": stats, "count": len(stats)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def enrich_players_bulk(
    ...
    try:
        ...
        enriched = await third_party_service.enrich_players_bulk(
            fpl_players, season, max_players
        )

        return {"players": enriched, "count": len(enriched), "season": season}
...
async def map_players_across_sources(
    season: str = "2025", db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        understat_service = UnderstatService(
            entity_resolution_service=entity_resolution, db_session=db
        )
        fbref_service = FBrefService(
            entity_resolution_service=entity_resolution, db_session=db
        )
        ...
        understat_mapping = await understat_service.map_to_fpl_players(
            understat_data, fpl_players, use_entity_resolution=True
        )
        fbref_mapping = await fbref_service.map_to_fpl_players(
            fbref_data, fpl_players, use_entity_resolution=True
        )

        # Calculate match statistics
        understat_high_confidence = sum(
            1 for m in understat_mapping.values() if m.get("confidence", 0.0) >= 0.85
        )
        fbref_high_confidence = sum(
            1 for m in fbref_mapping.values() if m.get("confidence", 0.0) >= 0.85
        )
        ...
        return {
            "understat_mapping": {
                ...
                "match_methods": {
                    "entity_resolution": sum(
                        1
                        for m in understat_mapping.values()
                        if m.get("match_method") == "entity_resolution"
                    ),
                    "fuzzy_fallback": sum(
                        1
                        for m in understat_mapping.values()
                        if m.get("match_method") == "fuzzy_fallback"
                    ),
                    "simple_name_match": sum(
                        1
                        for m in understat_mapping.values()
                        if m.get("match_method") == "simple_name_match"
                    ),
                },
...
async def resolve_players_bulk(request: BulkResolutionRequest):
    ...
    try:
        ...
        resolved = entity_resolution.resolve_players_bulk(
            players_data, include_fuzzy=request.include_fuzzy
        )
...
async def get_unmatched_players():
    ...
    try:
        ...
        return {"unmatched_players": unmatched, "count": len(unmatched)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def override_mapping(
    request: OverrideMappingRequest, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=409, detail=f"Database constraint violation: {str(e)}"
        )
    except Exception as e:
...
async def fuzzy_match_player(player_name: str, threshold: float = 0.8):
    """
    ...
    try:
        ...
        if entity_resolution.master_map is not None:
            if "FPL_Name" in entity_resolution.master_map.columns:
                all_names.extend(
                    entity_resolution.master_map["FPL_Name"].dropna().tolist()
                )
            if "Understat_Name" in entity_resolution.master_map.columns:
                all_names.extend(
                    entity_resolution.master_map["Understat_Name"].dropna().tolist()
                )
        ...
        return {
            ...
            "threshold": threshold,
            "matches": [
                {"name": name, "similarity": score} for name, score in matches[:10]
            ],  # Top 10
        }
...
async def resolve_all_players(
    store_mappings: bool = True, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        report = entity_resolution.resolve_all_players(
            db=db, fpl_players=fpl_players, store_mappings=store_mappings
        )
...
async def normalize_dgw_points(
    points: float, matches_played: int, gameweek_type: str = "normal"
):
    """
    ...
    try:
        normalized = data_cleaning.normalize_dgw_points(
            points, matches_played, gameweek_type
        )
...
async def calculate_defcon_metrics(player_data: Dict, position: str, minutes: int = 90):
    """
...
async def convert_metrics_to_float(
    data: Dict, metric_columns: Optional[List[str]] = None
):
    """
    ...
    try:
        ...
        return {"converted_data": converted, "conversions": conversions}
    ...
    try:
        if background_tasks:
            ...
            background_tasks.add_task(
                etl_service.sync_from_fpl_api, fpl_api, gameweek, season
            )
            return {
                "status": "started",
            ...
            # Run synchronously
            result = await etl_service.sync_from_fpl_api(fpl_api, gameweek, season)
...
async def bulk_upsert_players(players_data: List[Dict], batch_size: int = 100):
    """
...
async def bulk_upsert_stats(stats_data: List[Dict], batch_size: int = 100):
    """
...
async def get_etl_status():
    ...
    try:
        ...
        return {"scheduler_running": scheduler.running, "scheduled_jobs": jobs}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
