...
class DynamicFormAlpha:
    ...
    def optimize_alpha(
        ...
        logger.info(f"Starting Bayesian Optimization for alpha (n_calls={n_calls}, lookback={lookback_weeks})")
        ...
        try:
            ...
            if len(optimization_history) >= patience:
                ...
                recent_improvements = [
                    optimization_history[i]['rmse'] - optimization_history[i-1]['rmse']
                    for i in range(len(optimization_history) - patience + 1, len(optimization_history))
                    if i > 0
                ]
    ...
    def load_optimal_alpha_from_db(self, db: Session, gameweek: Optional[int] = None) -> Optional[float]:
        """
        ...
        try:
            ...
            if gameweek:
                entry = db.query(FormAlpha).filter(FormAlpha.gameweek == gameweek).first()
            else:
                # Get latest entry
            ...
            if entry:
                ...
                logger.info(f"Loaded optimal alpha {self.optimal_alpha:.4f} from database (GW {entry.gameweek})")
    ...
    def compare_with_baseline(
        ...
        optimized_rmse = self._calculate_rmse(self.optimal_alpha, historical_data, lookback_weeks)
        baseline_rmse = self._calculate_rmse(baseline_alpha, historical_data, lookback_weeks)
        
        improvement = ((baseline_rmse - optimized_rmse) / baseline_rmse) * 100 if baseline_rmse > 0 else 0
...
class DixonColesFDR:
    ...
    def get_stochastic_fdr(
        ...
        goal_probs_against = [poisson.pmf(i, exp_goals_against) for i in range(max_goals + 1)]
        ...
        goal_distribution = {
            ...
            'goals_against': {i: float(goal_probs_against[i]) for i in range(max_goals + 1)}
        }
    ...
    def compare_with_fpl_fdr(
        self,
        fixtures: List[Dict],
        fpl_fdr_data: Optional[Dict] = None
    ) -> Dict:
        """
    ...
    def verify_with_actual_outcomes(
        self,
        fixtures: List[Dict]
    ) -> Dict:
        """
        ...
        pred_goals_for = [p['home_goals_for'] for p in predictions] + [p['away_goals_for'] for p in predictions]
        actual_goals_for = [a['home_goals_for'] for a in actuals] + [a['away_goals_for'] for a in actuals]
        
        pred_goals_against = [p['home_goals_against'] for p in predictions] + [p['away_goals_against'] for p in predictions]
        actual_goals_against = [a['home_goals_against'] for a in actuals] + [a['away_goals_against'] for a in actuals]
        
        corr_for = float(np.corrcoef(pred_goals_for, actual_goals_for)[0, 1]) if len(pred_goals_for) > 1 else 0.0
        corr_against = float(np.corrcoef(pred_goals_against, actual_goals_against)[0, 1]) if len(pred_goals_against) > 1 else 0.0
...
class FeatureEngineeringService:
    ...
    def calculate_all_features(
        ...
        if historical_points:
            ...
            features['form_alpha_used'] = self.form_alpha.optimal_alpha if self.form_alpha.optimal_alpha is not None else 0.5
        else:
        ...
        if fixture_data and self.fdr_model.is_fitted:
            ...
            opponent = fixture_data.get('opponent', fixture_data.get('opponent_team', ''))
        ...
        if third_party_data:
            ...
            features['understat_xg'] = third_party_data.get('understat_xg', player_data.get('xg', 0.0))
            features['understat_xa'] = third_party_data.get('understat_xa', player_data.get('xa', 0.0))
            features['understat_npxg'] = third_party_data.get('understat_npxg', 0.0)
            features['understat_xg_per_90'] = third_party_data.get('understat_xg_per_90', player_data.get('xg_per_90', 0.0))
            features['understat_xa_per_90'] = third_party_data.get('understat_xa_per_90', player_data.get('xa_per_90', 0.0))
            features['understat_npxg_per_90'] = third_party_data.get('understat_npxg_per_90', 0.0)
            
            # FBref defensive metrics
            features['fbref_blocks'] = third_party_data.get('fbref_blocks', 0)
            features['fbref_blocks_per_90'] = third_party_data.get('fbref_blocks_per_90', 0.0)
            features['fbref_interventions'] = third_party_data.get('fbref_interventions', 0)
            features['fbref_interventions_per_90'] = third_party_data.get('fbref_interventions_per_90', 0.0)
            features['fbref_passes'] = third_party_data.get('fbref_passes', 0)
            features['fbref_passes_per_90'] = third_party_data.get('fbref_passes_per_90', 0.0)
        else:
    ...
    def _validate_and_fill_features(self, features: Dict[str, float]) -> Dict[str, float]:
        ...
        for key, value in features.items():
            ...
            if value is None or (isinstance(value, float) and (np.isnan(value) or np.isinf(value))):
                ...
                validated[key] = float(value) if isinstance(value, (int, float, np.number)) else 0.0
    ...
    def enrich_training_data(
        ...
        for idx, row in training_data.iterrows():
            ...
            # Get fixture data
            fixture_data = {
                'opponent': player_data.get('opponent_team', player_data.get('opponent', '')),
                'is_home': bool(player_data.get('was_home', player_data.get('is_home', True))),
            ...
            if self.third_party_service:
                ...
                third_party_data = {
                    'understat_xg': player_data.get('understat_xg', player_data.get('xg', 0.0)),
                    'understat_xa': player_data.get('understat_xa', player_data.get('xa', 0.0)),
                    'understat_npxg': player_data.get('understat_npxg', 0.0),
                    'understat_xg_per_90': player_data.get('understat_xg_per_90', player_data.get('xg_per_90', 0.0)),
                    'understat_xa_per_90': player_data.get('understat_xa_per_90', player_data.get('xa_per_90', 0.0)),
                    'understat_npxg_per_90': player_data.get('understat_npxg_per_90', 0.0),
                    ...
                    'fbref_interventions_per_90': player_data.get('fbref_interventions_per_90', 0.0),
        ...
        if null_counts.any():
            logger.warning(f"Found null values in enriched data: {null_counts[null_counts > 0].to_dict()}")
            # Fill remaining nulls
            enriched_df = enriched_df.fillna(0.0)
        
        logger.info(f"Enriched training data: {len(enriched_df)} rows, {len(enriched_df.columns)} columns")
