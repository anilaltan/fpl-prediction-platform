...
class PLEngine:
    ...
    def _ensure_models_loaded_sync(self) -> None:
        ...
        if not self.xmins_strategy.is_loaded:
            ...
            asyncio.run(self.model_loader.load_model(self.xmins_strategy, self.model_path))
        if not self.attack_strategy.is_loaded:
            asyncio.run(self.model_loader.load_model(self.attack_strategy, self.model_path))
        if not self.defense_strategy.is_loaded:
            asyncio.run(self.model_loader.load_model(self.defense_strategy, self.model_path))
    ...
    async def async_save_models(self, model_path: Optional[str] = None) -> None:
        ...
        try:
            # Prepare model data
            model_data = {
                'xmins_model': self.xmins_strategy.model if self.xmins_strategy.is_trained else None,
                'xmins_scaler': self.xmins_strategy.scaler,
                'xmins_feature_names': self.xmins_strategy.feature_names,
                'attack_xg_model': self.attack_strategy.xg_model if self.attack_strategy.xg_trained else None,
                'attack_xa_model': self.attack_strategy.xa_model if self.attack_strategy.xa_trained else None,
                'attack_scaler': self.attack_strategy.scaler,
                'defense_model': self.defense_strategy.pcs_model if self.defense_strategy.is_fitted else None,
    ...
    def calculate_expected_points(
        ...
        is_high_value_attacker = (
            (xg > 0.5 or xa > 0.3) or
            (ict_index > 50) or
            (player_data.get('form', 0) > 5.0)
        )
        
        xp_temp = xmins_factor * (goal_component + assist_component + cs_component + defcon_points_90)
        ...
        elif hasattr(self, '_historical_mean_actual') and hasattr(self, '_historical_mean_predicted'):
            if self._historical_mean_predicted > 0:
                conservative_scale = self._historical_mean_actual / self._historical_mean_predicted
    ...
    def train(
        ...
        if attack_features is not None and attack_xg_labels is not None and attack_xa_labels is not None:
            if len(attack_features) > 0 and len(attack_xg_labels) > 0 and len(attack_xa_labels) > 0:
                self.attack_strategy.train(attack_features, attack_xg_labels, attack_xa_labels)
                gc.collect()
    ...
    def fit_calibration(
        ...
        if method == 'linear':
            ...
            if var_pred > 0:
                ...
                self.calibration_scale = mean_actual / mean_pred if mean_pred > 0 else 1.0
            ...
            logger.info(f"Linear calibration fitted: scale={self.calibration_scale:.3f}, offset={self.calibration_offset:.3f}")
        
        self.calibration_fitted = True
        
        # Calculate metrics
        calibrated_pred = (predicted_arr * self.calibration_scale) + self.calibration_offset
        ...
        return {
            ...
            'improvement_pct': float((rmse_before - rmse_after) / rmse_before * 100) if rmse_before > 0 else 0.0
        }
    ...
    def models_loaded(self) -> bool:
        ...
        return (
            self.xmins_strategy.is_loaded and
            self.attack_strategy.is_loaded and
            self.defense_strategy.is_loaded
        )
