...
class XMinsStrategy(ModelInterface):
    ...
    def train(
        ...
        self._is_trained = True
        model_type = "XGBoost Classifier" if XGBOOST_AVAILABLE else "RandomForestClassifier"
        logger.info(f"xMins model ({model_type}) trained successfully on {len(X)} samples")
    ...
    def evaluate(
        ...
        try:
            ...
            logger.info(f"xMins model evaluation: Log Loss={log_loss_score:.4f}, AUC-ROC={auc_roc_score:.4f}, Accuracy={accuracy:.4f}")
    ...
    def predict(self, player_data: Dict, fixture_data: Optional[Dict] = None) -> Dict[str, float]:
        """
