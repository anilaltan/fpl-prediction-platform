...
class AttackStrategy(ModelInterface):
    ...
    async def load(self, model_path: Optional[str] = None) -> None:
        ...
        if model_path and os.path.exists(model_path):
            try:
                ...
                logger.warning(f"Failed to load Attack model from {model_path}: {str(e)}")
                self._initialize_empty_model()
        else:
            self._initialize_empty_model()
    ...
    def train(
        ...
        if optimize_hyperparameters and LIGHTGBM_AVAILABLE:
            ...
            best_params_xg = self._optimize_hyperparameters(X, y_xg, categorical_indices, validation_data)
            best_params_xa = self._optimize_hyperparameters(X, y_xa, categorical_indices, validation_data)
            
            self.xg_model = lgb.LGBMRegressor(**best_params_xg, random_state=42, verbose=-1, n_jobs=1)
            self.xa_model = lgb.LGBMRegressor(**best_params_xa, random_state=42, verbose=-1, n_jobs=1)
        ...
        if validation_data is not None and LIGHTGBM_AVAILABLE:
            ...
            if LIGHTGBM_AVAILABLE and categorical_indices:
                self.xg_model.fit(X_scaled, y_xg, categorical_feature=categorical_indices)
                self.xa_model.fit(X_scaled, y_xa, categorical_feature=categorical_indices)
            else:
        ...
        logger.info(f"Attack model ({model_type} xG/xA) trained successfully on {len(X)} samples")
    ...
    def evaluate(
        ...
        try:
            ...
            logger.info(f"Attack model evaluation: xG MAE={xg_mae:.4f}, xA MAE={xa_mae:.4f}")
