...
class DefenseStrategy(ModelInterface):
            ...
            try:
                ...
                if model_data.get('defense_model'):
                    ...
                    if hasattr(defense_model, 'pcs_model'):
                        ...
                        self.feature_names = getattr(defense_model, 'feature_names', None)
                    else:
                ...
                logger.warning(f"Failed to load Defense model from {model_path}: {str(e)}")
                self._initialize_empty_model()
        else:
            self._initialize_empty_model()
    ...
    def train(
        ...
        if categorical_features and feature_names and LIGHTGBM_AVAILABLE:
            categorical_indices = [
                feature_names.index(feat) for feat in categorical_features 
                if feat in feature_names
            ]
            if categorical_indices:
                logger.info(f"Treating {len(categorical_indices)} features as categorical: {categorical_features}")
        
        # Hyperparameter optimization
        if optimize_hyperparameters and LIGHTGBM_AVAILABLE:
            logger.info("Optimizing hyperparameters using grid search...")
            best_params = self._optimize_hyperparameters(X, y, categorical_indices, validation_data)
            
            self.pcs_model = lgb.LGBMClassifier(**best_params, random_state=42, verbose=-1, n_jobs=1, objective='binary')
        ...
        logger.info(f"Defense model ({model_type} P_CS) trained successfully on {len(X)} samples")
    ...
    def evaluate(
        ...
        try:
            ...
            logger.info(f"Defense model evaluation: Log Loss={log_loss_score:.4f}, AUC-ROC={auc_roc_score:.4f}, Accuracy={accuracy:.4f}")
    ...
    def predict(
        ...
        if player_data:
            ...
            defcon_points = self.calculate_defcon_points(player_data, position, expected_minutes)
    ...
    def calculate_defcon_points(
        ...
        blocks_per_90 = float(player_data.get('blocks_per_90', player_data.get('avg_blocks', 0.0)))
        interventions_per_90 = float(player_data.get('interventions_per_90', player_data.get('avg_interventions', 0.0)))
        passes_per_90 = float(player_data.get('passes_per_90', player_data.get('avg_passes', 0.0)))
