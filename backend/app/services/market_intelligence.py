...
class MarketIntelligenceService:
    ...
    def calculate_player_ranks(
        ...
        if not predictions:
            logger.warning(f"No predictions found for gameweek {gameweek}, season {season}")
            return pd.DataFrame()
        ...
        df['ownership_rank'] = df['ownership'].rank(method='dense', ascending=False).astype(int)
    ...
    async def _get_ownership_from_api_async(self, fpl_id: int) -> Optional[float]:
        ...
        try:
            ...
            logger.warning(f"Failed to fetch ownership from FPL API for player {fpl_id}: {str(e)}")
            return None
    ...
    def _get_ownership_batch_from_api(self, fpl_ids: List[int]) -> Dict[int, float]:
        ...
        try:
            ...
            if loop.is_running():
                ...
                logger.warning("Cannot use sync API call in async context, returning empty ownership map")
                return {}
        except RuntimeError:
        ...
        try:
            ...
            for element in elements:
                ...
                if element_id in fpl_ids:
                    ownership_map[element_id] = float(element.get('selected_by_percent', 0.0))
    ...
    def validate_ranks(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        if xp_ranks != expected_xp_ranks:
            errors.append(f"xP ranks have gaps: found {xp_ranks}, expected {expected_xp_ranks}")
        ...
        if ownership_ranks != expected_ownership_ranks:
            errors.append(f"Ownership ranks have gaps: found {ownership_ranks}, expected {expected_ownership_ranks}")
    ...
    def calculate_arbitrage_scores_and_categories(
        ...
        logger.info(f"Arbitrage score range: [{df['arbitrage_score'].min():.1f}, {df['arbitrage_score'].max():.1f}]")
    ...
    def persist_market_intelligence(
        ...
        # Validate required columns
        required_columns = ['player_id', 'xp_rank', 'ownership_rank', 'arbitrage_score', 'category']
        missing_columns = [col for col in required_columns if col not in df.columns]
        if missing_columns:
            raise ValueError(f"DataFrame missing required columns: {missing_columns}")
        ...
        try:
            ...
            existing_count_before = db.query(MarketIntelligence).filter(
                and_(
    ...
    def calculate_and_persist_market_intelligence(
        ...
        """
        logger.info(f"Starting market intelligence calculation and persistence for GW{gameweek}, {season}")
        ...
        if df.empty:
            logger.warning(f"No player data found for gameweek {gameweek}, season {season}")
            return {
                'success': False,
        ...
        logger.info(f"Market intelligence calculation and persistence complete: {result}")
    ...
    def validate_market_intelligence_output(
        ...
        logger.info(f"Starting market intelligence validation for GW{gameweek}, {season}")
        
        try:
            # Step 1: Get all market intelligence records for this gameweek
            mi_records = db.query(MarketIntelligence).filter(
                and_(
                    MarketIntelligence.gameweek == gameweek,
                    MarketIntelligence.season == season
                )
            ...
            if not mi_records:
                errors.append(f"No market intelligence records found for gameweek {gameweek}, season {season}")
                return {
                    'is_valid': False,
                    'errors': errors,
                    'warnings': warnings,
            ...
            high_ownership_players = db.query(Player).filter(
                Player.ownership > ownership_threshold
            ).all()
            
            logger.info(f"Found {len(high_ownership_players)} players with ownership > {ownership_threshold}%")
            ...
            for player in high_ownership_players:
                ...
                mi_record = next(
                    (mi for mi in mi_records if mi.player_id == player.id),
                    None
                )
                ...
                current_prediction = db.query(Prediction).filter(
                    and_(
                ...
                ).order_by(Prediction.gameweek.desc()).limit(min_previous_gameweeks).all()
                ...
                if len(previous_predictions) >= min_previous_gameweeks:
                    ...
                    previous_avg_xp = sum(float(p.xp) for p in previous_predictions) / len(previous_predictions)
                ...
                should_be_overvalued = (
                    float(player.ownership) > ownership_threshold and
                ...
                if float(player.ownership) > ownership_threshold and has_declining_trend:
            ...
            overvalued_mi_records = [mi for mi in mi_records if mi.category == 'Overvalued']
            
            for mi_record in overvalued_mi_records:
                player = db.query(Player).filter(Player.id == mi_record.player_id).first()
                if not player:
                    warnings.append(f"Market intelligence record references non-existent player_id {mi_record.player_id}")
                    continue
                
                if float(player.ownership) <= ownership_threshold and mi_record.arbitrage_score <= 0:
                    warnings.append(
            ...
            if is_valid:
                ...
                logger.warning(f"✗ Market intelligence validation failed for GW{gameweek}")
