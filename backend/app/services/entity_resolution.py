...
class EntityResolutionService:
    ...
    async def load_master_map(self, force_reload: bool = False) -> bool:
        ...
        try:
            ...
            if not force_reload and os.path.exists(self.LOCAL_MAP_PATH):
                file_age = datetime.now().timestamp() - os.path.getmtime(
                    self.LOCAL_MAP_PATH
                )
                # Use local if less than 24 hours old
                if file_age < 86400:
                    logger.info(
                        f"Loading Master ID Map from local cache: {self.LOCAL_MAP_PATH}"
                    )
    ...
    def generate_canonical_name(
        ...
        if (
            understat_name
            and not pd.isna(understat_name)
            and str(understat_name).strip()
        ):
        ...
        fpl_entry = next(
            (name for source, name in available_names if source == "fpl"), None
        )
        if fpl_entry:
            ...
            for col in ["FPL_Name", "Understat_Name", "FBref_Name"]:
                if col in self.master_map.columns:
                    matches = self.master_map[
                        self.master_map[col].apply(
                            lambda x: self.normalize_name(x) == fpl_norm
                        )
                    ]
    ...
    def fuzzy_match(
        ...
        norm_to_original = {self.normalize_name(c): c for c in candidate_names}
    ...
    def fuzzy_match_multi_source(
        ...
        if understat_names:
            ...
            for norm_candidate, score_100 in understat_matches:
                original_candidate = norm_to_original_understat.get(
        ...
        if fbref_names:
            ...
            # Map back to original names and convert scores to 0-1 scale
            norm_to_original_fbref = {self.normalize_name(c): c for c in fbref_names}

            for norm_candidate, score_100 in fbref_matches:
                original_candidate = norm_to_original_fbref.get(
                    norm_candidate, norm_candidate
                )
                score_01 = score_100 / 100.0
    ...
    def resolve_player_entity(
            ...
            if "FBref" in master_entry and pd.notna(master_entry["FBref"]):
                try:
                    ...
                    if pd.notna(fbref_val):
                        result["fbref_id"] = (
                            str(fbref_val)
                            if not isinstance(fbref_val, str)
                            else fbref_val
                        )
                except (ValueError, TypeError):
                    pass
        ...
        if self.master_map is not None:
            ...
            if best_match:
                result["matched"] = True
                result["match_method"] = (
                    f"fuzzy_match_{match_source}" if match_source else "fuzzy_match"
                )
                ...
                if matched_entry:
                    if "FPL" in matched_entry and pd.notna(matched_entry["FPL"]):
                        try:
                            ...
                            pass
                    if "Understat" in matched_entry and pd.notna(
                        matched_entry["Understat"]
                    ):
                        try:
    ...
    def resolve_players_bulk(
        self, players: List[Dict], include_fuzzy: bool = True
    ) -> Dict[int, Dict]:
        """
        ...
        for player in players:
            ...
            fpl_name = (
                player.get("web_name") or player.get("name") or player.get("fpl_name")
            )
        ...
        if self.master_map is not None:
            ...
            if "FPL" in self.master_map.columns:
                ...
                if existing.empty:
                    ...
                    self.master_map = pd.concat(
                        [self.master_map, new_row], ignore_index=True
                    )
                    logger.info(
                        f"Added manual mapping for {fpl_name} (FPL ID: {fpl_id})"
                    )
                else:
                    # Update existing
    ...
    def upsert_mapping(
        ...
        try:
            ...
            existing = (
                db.query(EntityMapping).filter(EntityMapping.fpl_id == fpl_id).first()
            )

            if existing:
                # Update existing mapping
                existing.understat_name = (
                    understat_name if understat_name else existing.understat_name
                )
                ...
                db.refresh(existing)
                logger.info(
                    f"Updated entity mapping for FPL ID {fpl_id}: {canonical_name}"
                )
                return existing
            else:
                # Create new mapping
                ...
                logger.info(
                    f"Created entity mapping for FPL ID {fpl_id}: {canonical_name}"
                )
                return new_mapping
            ...
            db.rollback()
            logger.error(
                f"Integrity error upserting mapping for FPL ID {fpl_id}: {str(e)}"
            )
            raise
        except Exception as e:
            db.rollback()
    ...
    def get_mapping_by_canonical_name(
        ...
        return (
            ...
            .filter(EntityMapping.canonical_name == canonical_name)
            .first()
        )

    def get_mapping_by_understat_name(
        self, db: Session, understat_name: str
    ) -> Optional[EntityMapping]:
        """
        ...
        return (
            db.query(EntityMapping)
            .filter(EntityMapping.understat_name == understat_name)
            .first()
        )

    def get_mapping_by_fbref_name(
        self, db: Session, fbref_name: str
    ) -> Optional[EntityMapping]:
        """
        ...
        return (
            db.query(EntityMapping)
            .filter(EntityMapping.fbref_name == fbref_name)
            .first()
        )
    ...
    def get_all_mappings(
        ...
        if manually_verified_only:
            query = query.filter(EntityMapping.manually_verified == True)  # noqa: E712
    ...
    def update_confidence_score(
        self, db: Session, fpl_id: int, confidence_score: float
    ) -> Optional[EntityMapping]:
        """
        ...
        if mapping:
            ...
            db.refresh(mapping)
            logger.info(
                f"Updated confidence score for FPL ID {fpl_id}: {confidence_score}"
            )
            return mapping
        else:
    ...
    def mark_manually_verified(
        self, db: Session, fpl_id: int, verified: bool = True
    ) -> Optional[EntityMapping]:
        """
        ...
        if mapping:
            ...
            db.refresh(mapping)
            logger.info(
                f"Marked mapping for FPL ID {fpl_id} as {'verified' if verified else 'unverified'}"
            )
            return mapping
        else:
            logger.warning(f"No mapping found for FPL ID {fpl_id}")
    ...
    def override_mapping(
        ...
        if not existing_mapping:
            raise ValueError(
                f"No mapping found for FPL ID {fpl_id}. Create mapping first using upsert_mapping."
            )
        ...
        try:
            ...
            db.rollback()
            logger.error(
                f"Integrity error overriding mapping for FPL ID {fpl_id}: {str(e)}"
            )
            raise
        except Exception as e:
            db.rollback()
    ...
    def delete_mapping(self, db: Session, fpl_id: int) -> bool:
        """
    ...
    def resolve_all_players(
        self, db: Session, fpl_players: List[Dict], store_mappings: bool = True
    ) -> Dict:
        """
        ...
        for player in fpl_players:
            ...
            try:
                ...
                if resolution.get("matched"):
                    ...
                    if store_mappings:
                        try:
                            ...
                            if self.master_map is not None:
                                master_entry = self.find_player_in_master_map(
                                    fpl_id=fpl_id, fpl_name=fpl_name
                                )
                                if master_entry:
                                    if "Understat_Name" in master_entry and pd.notna(
                                        master_entry["Understat_Name"]
                                    ):
                                        understat_name = str(
                                            master_entry["Understat_Name"]
                                        )
                                    if "FBref_Name" in master_entry and pd.notna(
                                        master_entry["FBref_Name"]
                                    ):
                            ...
                            logger.error(
                                f"Error storing mapping for FPL ID {fpl_id}: {str(e)}"
                            )
                else:
                ...
                logger.error(
                    f"Error resolving player {fpl_name} (FPL ID {fpl_id}): {error_msg}"
                )
                report["unmatched_count"] += 1
                report["unmatched_players"].append(
                    {
        ...
        if report["total_players"] > 0:
            report["match_accuracy"] = (
                report["matched_count"] / report["total_players"]
            ) * 100.0
