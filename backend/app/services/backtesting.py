...
class BacktestingEngine:
    ...
    def expanding_window_backtest(
        ...
        min_weeks = min_train_weeks if min_train_weeks is not None else self.min_train_weeks
        ...
        for test_gw in gameweeks[min_weeks:]:
            ...
            try:
                ...
                logger.info(f"Gameweek {test_gw}: RMSE={week_metrics['rmse']:.2f}, MAE={week_metrics['mae']:.2f}")
    ...
    def rolling_window_backtest(
        ...
        if len(gameweeks) < window_size + 1:
            return {'error': f'Insufficient data. Need at least {window_size + 1} gameweeks'}
        ...
        for i in range(window_size, len(gameweeks)):
            ...
            test_data = historical_data[
                historical_data[gameweek_column] == test_gw
            ]
