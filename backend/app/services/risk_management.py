...
class OwnershipArbitrage:
    ...
    def find_differential_alternatives(
        ...
        for player in all_players:
            ...
            ownership_diff = (over_owned_player.get('ownership_percent', 0.0) / 100.0) - player_ownership
        ...
        for player in over_owned[:10]:  # Top 10 over-owned
            ...
            if alternatives:
                opportunities.append({
                    ...
                    'potential_gain': alternatives[0]['xp_improvement'] if alternatives else 0.0
                })
        
        # Calculate total arbitrage potential
        total_potential_gain = sum(
            opp['potential_gain'] for opp in opportunities
        )
...
class CaptainViceCaptainParadox:
    ...
    def calculate_weighted_expected_value(
        ...
        weighted_value = (
            (1 - c_no_play_prob) * (2 * c_xp) +
            c_no_play_prob * (2 * vc_xp)
        )
        ...
        swapped_weighted_value = (
            (1 - vc_no_play_prob) * (2 * vc_xp) +
    ...
    def select_optimal_captain_pair(
        ...
        for i, captain in enumerate(sorted_candidates[:5]):
            ...
            for j, vc in enumerate(sorted_candidates):
                ...
                analysis = self.calculate_weighted_expected_value(
                    captain, vc, gameweek
                )
...
class ChipTiming:
    ...
    def calculate_wildcard_value(
        ...
        for week in range(gameweek, min(gameweek + self.horizon_weeks, 39)):
            ...
            current_week_points = sorted([
                p.get(xp_key, p.get('expected_points', 0.0))
        ...
        return {
            ...
            'recommendation': (
                f"Play Wildcard" if should_play else
                f"Wait (gain: {gain:.1f} < threshold: {self.wildcard_threshold})"
            )
        }
    ...
    def calculate_bench_boost_value(
        ...
        avg_bench_xp = np.mean([p.get(xp_key, p.get('expected_points', 0.0)) for p in bench_players])
        ...
        return {
            ...
            'should_play': should_play,
            'recommendation': (
                f"Play Bench Boost (gain: {gain:.1f} points)" if should_play else
                f"Wait for better bench fixtures (current: {gain:.1f} < threshold: {self.bench_boost_threshold})"
            )
        }
    ...
    def calculate_free_hit_value(
        ...
        return {
            ...
            'should_play': should_play,
            'recommendation': (
                f"Play Free Hit (gain: {gain:.1f} points)" if should_play else
                f"Save Free Hit (gain: {gain:.1f} < threshold: {self.free_hit_threshold})"
            )
        }
    ...
    def analyze_all_chips(
        ...
        if playable_chips:
            ...
            playable_chips.sort(key=lambda x: x[1].get('expected_gain', 0.0), reverse=True)
            best_chip = playable_chips[0]
            analysis['recommendation'] = f"Play {best_chip[0].replace('_', ' ').title()}: {best_chip[1]['recommendation']}"
        else:
            analysis['recommendation'] = "No chips recommended at this time. Save for better opportunities."
...
class RiskManagementService:
    ...
    def get_comprehensive_analysis(
        ...
        return {
            'gameweek': gameweek,
            'ownership_arbitrage': self.arbitrage.analyze_arbitrage_opportunities(players, gameweek),
