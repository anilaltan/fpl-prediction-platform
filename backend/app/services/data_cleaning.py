...
class DataCleaningService:
    ...
    def calculate_defcon_floor_points(
        ...
        if use_raw_data:
            ...
            blocks = self._safe_float(
                player_data.get("blocks", player_data.get("total_blocks", 0))
            )
            tackles = self._safe_float(
                player_data.get("tackles", player_data.get("total_tackles", 0))
            )
            ...
            passes = self._safe_float(
                player_data.get(
                    "passes",
                    player_data.get(
                        "total_passes", player_data.get("successful_passes", 0)
                    ),
                )
            )

            # Calculate per-90 from raw data
            total_minutes = self._safe_float(
                player_data.get("minutes", player_data.get("minutes_played", 0))
            )
            games_played = self._safe_float(
                player_data.get("games", player_data.get("games_played", 1))
            )

            if total_minutes > 0 and games_played > 0:
                avg_minutes_per_game = total_minutes / games_played
                blocks_per_90 = (blocks / games_played) * (
                    90.0 / max(avg_minutes_per_game, 1.0)
                )
                interventions_per_90 = ((tackles + interceptions) / games_played) * (
                    90.0 / max(avg_minutes_per_game, 1.0)
                )
                ...
                interventions_per_90 = self._safe_float(
                    player_data.get("interventions_per_90", 0.0)
                )
    ...
    def _convert_dict_to_float(
        self, data: Dict, metric_columns: Optional[List[str]] = None
    ) -> Dict:
    ...
    def _convert_dataframe_to_float(
        self, df: pd.DataFrame, metric_columns: Optional[List[str]] = None
    ) -> pd.DataFrame:
    ...
    def clean_player_data(
        ...
        if calculate_defcon and position:
            ...
            floor_points = self.calculate_defcon_floor_points(
                cleaned, position, minutes, use_raw_data=True
            )
            cleaned["defcon_floor_points"] = floor_points
            cleaned["defcon_blocks_per_90"] = self._safe_float(
                cleaned.get("blocks_per_90", 0.0)
            )
            cleaned["defcon_interventions_per_90"] = self._safe_float(
                cleaned.get("interventions_per_90", 0.0)
            )
            cleaned["defcon_passes_per_90"] = self._safe_float(
                cleaned.get("passes_per_90", 0.0)
            )
    ...
    def get_defcon_metrics(self, player_data: Dict, position: str) -> Dict[str, float]:
        """
        ...
        return {
            "floor_points": self.calculate_defcon_floor_points(
                player_data, position, minutes
            ),
            "floor_points_90": self.calculate_defcon_floor_points(
            ...
            "passes_per_90": self._safe_float(player_data.get("passes_per_90", 0.0)),
            "defcon_score": self.calculate_defcon_floor_points(
                player_data, position, 90
            ),
        }
