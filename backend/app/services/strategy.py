...
class StrategyService:
    ...
    def __init__(self):
        ...
        self.xp_ownership_ratio_threshold = 0.3  # xP/Ownership ratio threshold for overvalued
    ...
    def analyze_ownership_arbitrage(
        ...
        for player in players:
            ...
            ownership_ratio = ownership_percent / 100.0 if ownership_percent > 0 else 0.0
            ...
            try:
                prediction = plengine.predict(
                    player_data=player,
                    fixture_data=player.get('fixture_data')
                )
                xp = prediction.get('expected_points', 0.0)
            except Exception as e:
                logger.warning(f"Error predicting xP for player {player.get('id')}: {str(e)}")
                xp = 0.0
            ...
            if xp_ownership_ratio < self.xp_ownership_ratio_threshold:
                value_score = xp / player.get('price', 50.0) if player.get('price', 0) > 0 else 0.0
                
                overvalued_players.append({
                    'player_id': player.get('id'),
                    'name': player.get('web_name', player.get('name', '')),
                    'position': self._get_position_name(player.get('element_type', 3)),
                    'team_id': player.get('team', 0),
                    'ownership_percent': ownership_percent,
                    'expected_points': xp,
                    'xp_ownership_ratio': xp_ownership_ratio,
                    'price': player.get('now_cost', 0) / 10.0,  # Convert from FPL units
    ...
    def calculate_captain_vice_captain_value(
        ...
        try:
            capt_prediction = plengine.predict(
                player_data=captain,
                fixture_data=captain.get('fixture_data')
            )
            xp_capt = capt_prediction.get('expected_points', 0.0)
            p_start_capt = capt_prediction.get('p_start', 0.7)
        except Exception as e:
            logger.warning(f"Error predicting for captain {captain.get('id')}: {str(e)}")
            xp_capt = 0.0
            p_start_capt = 0.7
        ...
        try:
            vc_prediction = plengine.predict(
                player_data=vice_captain,
                fixture_data=vice_captain.get('fixture_data')
            )
            xp_vc = vc_prediction.get('expected_points', 0.0)
            p_start_vc = vc_prediction.get('p_start', 0.7)
        except Exception as e:
            logger.warning(f"Error predicting for vice-captain {vice_captain.get('id')}: {str(e)}")
            xp_vc = 0.0
            p_start_vc = 0.7
    ...
    def analyze_chip_triggers(
        ...
        recommendations['analysis'] = {
            ...
            'recommendation': 'Use Wildcard' if recommendations['use_wildcard'] else 'Keep Current Squad'
        }
        ...
        for player in squad:
            ...
            for week in range(1, horizon_weeks + 1):
                try:
                    ...
                    logger.warning(f"Error predicting for player {player.get('id')} week {week}: {str(e)}")
                    # Use fallback
    ...
    def generate_strategy_notes(
        ...
        try:
            ...
            optimized_player_ids = {p.get('id') for p in solver_solution.get('squad_week1', [])}
            overvalued_ids = {p['player_id'] for p in ownership_analysis.get('overvalued_players', [])}
        ...
        try:
            ...
            if captain and vice_captain:
                ...
                capt_data = next(
                    (p for p in players if p.get('id') == captain.get('id')),
                    captain
                )
                vc_data = next(
