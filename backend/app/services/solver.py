...
class FPLSolver:
    ...
    def _create_ilp_model(
        ...
        points_component = pulp.lpSum([
            ...
            pulp.lpSum([
                player_dict[i].get('expected_points', [0.0] * self.horizon_weeks)[w - 1] * y[(i, w)]
                for i in player_ids
            ])
            for w in weeks
        ])
        ...
        for w in weeks:
            ...
            extra_transfers_vars[w] = pulp.LpVariable(f"extra_transfers_w{w}", lowBound=0, cat='Continuous')
            
            if w == 1:
                # Week 1: transfers from current squad
                if current_squad:
                    transfers_in = pulp.lpSum([t[(i, w)] for i in player_ids if i not in current_squad])
            ...
            transfer_penalty_component += self.transfer_penalty * extra_transfers_vars[w]
                ...
                if current_squad:
                    for i in player_ids:
                        if i in current_squad:
                            ...
                            prob += u[(i, w)] >= (1 - x[(i, w)])
                            prob += t[(i, w)] == 0  # Can't transfer in from current squad
                        else:
                            # If player not in current squad but in week 1 squad, transfer in
                            prob += t[(i, w)] >= x[(i, w)]
                            prob += u[(i, w)] == 0  # Can't transfer out if not in current squad
                else:
    ...
    def _extract_solution(
        ...
        for w in weeks:
            ...
            for player_id in player_dict.keys():
                ...
                if pulp.value(x[(player_id, w)]) == 1:
                    player_info = {
                        ...
                        'expected_points': player_dict[player_id].get('expected_points', [0.0] * self.horizon_weeks)[w - 1]
                    }
            ...
            if w > 1:
                ...
                solution['total_transfer_cost'] += solution['transfers_by_week'][f'week_{w}']['transfer_cost']
            ...
            if w == 1 and current_squad:
                ...
                if transfers_in_w1 or transfers_out_w1:
                    ...
                    extra_transfers_w1 = max(0, total_transfers_w1 - self.free_transfers)
                    ...
                    solution['total_transfer_cost'] += solution['transfers_by_week']['week_1']['transfer_cost']
            
            solution['total_expected_points'] += (self.discount_factor ** (w - 1)) * week_points
    ...
    def _select_captain_vice_captain(
        ...
        for player in starting_xi:
            ...
            expected_points = player_info.get('expected_points', [0.0] * self.horizon_weeks)[0]
    ...
    def optimize_from_plengine(
        ...
        for player in players_data:
            ...
            for week in range(1, self.horizon_weeks + 1):
                ...
                player_fixture = next(
                    (f for f in week_fixtures if f.get('player_id') == player_id),
                    None
                )
                ...
                try:
                    prediction = plengine.predict(
                        player_data=player,
                        fixture_data=player_fixture
                    )
                    
                    expected_points.append(prediction['expected_points'])
                    p_start_list.append(prediction.get('p_start', 0.7))
                except Exception as e:
                    logger.warning(f"Error predicting for player {player_id} week {week}: {str(e)}")
