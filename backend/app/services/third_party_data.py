...
class UnderstatService:
    ...
    def __init__(self, entity_resolution_service=None, db_session: Optional[Session] = None):
        ...
        try:
                ...
                for pattern in patterns:
                    try:
                        ...
                        if json_match:
                            ...
                            elif isinstance(parsed_data, dict) and 'players' in parsed_data:
                                players_data = parsed_data['players']
                            elif isinstance(parsed_data, dict):
                                # If it's a dict with player data, try to extract
                                players_data = list(parsed_data.values()) if parsed_data else []
                            
                            if players_data:
                                break
                    except (json.JSONDecodeError, AttributeError) as e:
                        logger.debug(f"Failed to parse with pattern {pattern}: {str(e)}")
                        continue
            ...
            for player in players_data:
                try:
                    ...
                    name = player.get('player_name') or player.get('name') or player.get('Player') or ''
                    team = player.get('team_title') or player.get('team') or player.get('Team') or ''
                    position = player.get('position') or player.get('Position') or ''
                    
                    # Parse numeric fields with error handling
                    games = self._safe_int(player.get('games') or player.get('Games') or 0)
                    time = self._safe_int(player.get('time') or player.get('Time') or player.get('minutes') or 0)
                    goals = self._safe_int(player.get('goals') or player.get('Goals') or 0)
                    assists = self._safe_int(player.get('assists') or player.get('Assists') or 0)
                    xg = self._safe_float(player.get('xG') or player.get('xg') or 0.0)
                    xa = self._safe_float(player.get('xA') or player.get('xa') or 0.0)
                    npxg = self._safe_float(player.get('npxG') or player.get('npxg') or player.get('NPxG') or 0.0)
            ...
            logger.info(f"Successfully extracted {len(structured_data)} players from Understat")
            return structured_data
            
        except httpx.HTTPStatusError as e:
            logger.error(f"HTTP error fetching Understat data: {e.response.status_code}")
            return []
        except Exception as e:
    ...
    async def map_to_fpl_players(
        ...
        if use_entity_resolution and self.entity_resolution:
            ...
            understat_names = [p.get('name', '') for p in understat_data if p.get('name')]
            ...
            for fpl_player in fpl_players:
                ...
                if resolution.get('matched'):
                    ...
                    if 'Understat_Name' in resolution:
                        understat_name = resolution.get('Understat_Name')
                    ...
                    if understat_names:
                        ...
                        if matches.get('understat'):
                            ...
                            understat_player = next(
                                (p for p in understat_data if p.get('name') == best_match_name),
                                None
                            )
        ...
        if not mappings:
            mappings = self.name_matcher.create_mapping(fpl_players, understat_data, threshold=0.7)
...
class FBrefService:
    ...
    async def get_player_defensive_stats(
        ...
        try:
            ...
            defensive_table = soup.find('table', {'id': re.compile(r'.*defensive.*', re.I)})
            if defensive_table:
                tables.append(defensive_table)
            
            # Strategy 2: Look for standard stats table
            if not tables:
                stats_tables = soup.find_all('table', {'class': re.compile(r'stats.*table', re.I)})
            ...
            if not tables:
                ...
                for table in all_tables:
                    ...
                    if any(keyword in header_text for keyword in ['tackle', 'interception', 'block', 'defensive']):
                        tables.append(table)
                        break
            ...
            for table in tables:
                ...
                games_idx = self._find_column_by_keywords(header_map, ['games', 'mp', 'matches'])
                minutes_idx = self._find_column_by_keywords(header_map, ['minutes', 'min', 'mins'])
                blocks_idx = self._find_column_by_keywords(header_map, ['blocks', 'blocked', 'blk'])
                tackles_idx = self._find_column_by_keywords(header_map, ['tackles', 'tkl', 'tklw'])
                interceptions_idx = self._find_column_by_keywords(header_map, ['interceptions', 'int', 'inter'])
                passes_idx = self._find_column_by_keywords(header_map, ['passes', 'pass', 'passes_completed'])
                ...
                for row in rows:
                    ...
                    try:
                        ...
                        name_cell = cells[name_idx] if name_idx is not None else cells[0]
                        ...
                        stats = {
                            ...
                            'position': cells[pos_idx].get_text(strip=True) if pos_idx is not None and len(cells) > pos_idx else '',
                            'games': self._parse_int(cells[games_idx].get_text(strip=True)) if games_idx is not None and len(cells) > games_idx else 0,
                            'minutes': self._parse_int(cells[minutes_idx].get_text(strip=True)) if minutes_idx is not None and len(cells) > minutes_idx else 0,
                        }
                        
                        # Extract defensive metrics
                        stats['blocks'] = self._parse_int(cells[blocks_idx].get_text(strip=True)) if blocks_idx is not None and len(cells) > blocks_idx else 0
                        stats['tackles'] = self._parse_int(cells[tackles_idx].get_text(strip=True)) if tackles_idx is not None and len(cells) > tackles_idx else 0
                        stats['interceptions'] = self._parse_int(cells[interceptions_idx].get_text(strip=True)) if interceptions_idx is not None and len(cells) > interceptions_idx else 0
                        stats['passes'] = self._parse_int(cells[passes_idx].get_text(strip=True)) if passes_idx is not None and len(cells) > passes_idx else 0
                        
                        # Calculate per-90 metrics
                        minutes = stats.get('minutes', 0)
                        if minutes > 0:
                            stats['blocks_per_90'] = round((stats.get('blocks', 0) / minutes) * 90, 3)
                            stats['tackles_per_90'] = round((stats.get('tackles', 0) / minutes) * 90, 3)
                            stats['interceptions_per_90'] = round((stats.get('interceptions', 0) / minutes) * 90, 3)
                            stats['passes_per_90'] = round((stats.get('passes', 0) / minutes) * 90, 3)
                            ...
                            stats['interventions_per_90'] = round((interventions / minutes) * 90, 3)
                        else:
            ...
            if player_name:
                defensive_stats = [
                    p for p in defensive_stats 
                    if player_name.lower() in p['name'].lower()
                ]
            
            logger.info(f"Successfully extracted {len(defensive_stats)} players from FBref")
    ...
    def _find_column_by_keywords(self, header_map: Dict[int, str], keywords: List[str]) -> Optional[int]:
    ...
    async def map_to_fpl_players(
            ...
            for fpl_player in fpl_players:
                ...
                if resolution.get('matched'):
                    ...
                    if 'FBref_Name' in resolution:
                        fbref_name = resolution.get('FBref_Name')
        ...
        # If no Entity Resolution or no matches, fall back to simple name matching
        if not mappings:
            mappings = self.name_matcher.create_mapping(fpl_players, fbref_data, threshold=0.7)
            # Update match_method for simple matches
            for fpl_id in mappings:
                if 'match_method' not in mappings[fpl_id]:
                    mappings[fpl_id]['match_method'] = 'simple_name_match'
    ...
    async def enrich_player_data(
        ...
        try:
            ...
            if understat_data:
                ...
                mapping = await self.understat.map_to_fpl_players(understat_data, [fpl_player])
                if fpl_player.get('id') in mapping:
                    ...
                    if self.entity_resolution and self.db_session:
                        try:
                            ...
                            logger.warning(f"Failed to store Understat mapping: {str(e)}")
        except Exception as e:
