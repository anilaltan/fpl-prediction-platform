...
class ETLService:
    ...
    def __init__(self) -> None:
        ...
        self.async_engine = create_async_engine(async_url, echo=False, pool_pre_ping=True)
    ...
    async def upsert_player(
        ...
        try:
            ...
            logger.error(f"Error UPSERTing player {player_data.get('fpl_id')}: {str(e)}")
            raise
        finally:
            if should_close:
                await session.close()
    ...
    async def upsert_player_gameweek_stats(
        ...
        try:
            ...
            stats_dict = {
                ...
                'goals': int(stats_data.get('goals_scored', stats_data.get('goals', 0))),
                ...
                'total_points': int(stats_data.get('total_points', stats_data.get('points', 0))),
                'normalized_points': float(stats_data.get('normalized_points', stats_data.get('total_points', 0))),
                'xg': float(stats_data.get('expected_goals', stats_data.get('xg', 0))),
                'xa': float(stats_data.get('expected_assists', stats_data.get('xa', 0))),
                'xgi': float(stats_data.get('expected_goal_involvements', stats_data.get('xgi', 0))),
                'xgc': float(stats_data.get('expected_goals_conceded', stats_data.get('xgc', 0))),
                'npxg': float(stats_data.get('npxg', stats_data.get('expected_goals', 0))),
                ...
                'passes': int(stats_data.get('passes', stats_data.get('successful_passes', 0))),
                'defcon_floor_points': float(stats_data.get('defcon_floor_points', 0)),
                'was_home': bool(stats_data.get('was_home', True)),
                'opponent_team': int(stats_data.get('opponent_team', 0)) if stats_data.get('opponent_team') else None,
                'team_score': int(stats_data.get('team_score', 0)) if stats_data.get('team_score') else None,
                'opponent_score': int(stats_data.get('opponent_score', 0)) if stats_data.get('opponent_score') else None,
            }
            ...
            logger.debug(f"UPSERTed stats: FPL ID {fpl_id}, GW {gameweek}, Season {season}")
            
            return stats_dict
            
        except Exception as e:
            await session.rollback()
            logger.error(f"Error UPSERTing stats for FPL ID {stats_data.get('fpl_id')}, GW {stats_data.get('gameweek')}: {str(e)}")
            raise
        finally:
            if should_close:
                await session.close()
            ...
            for i in range(0, len(players_data), batch_size):
                ...
                for player_data in batch:
                    try:
                        ...
                        logger.error(f"Error in bulk upsert for player {player_data.get('fpl_id')}: {str(e)}")
                        errors += 1
                
                # Commit batch
                await session.commit()
                logger.info(f"Processed batch {i // batch_size + 1}: {len(batch)} players")
    ...
    async def bulk_upsert_gameweek_stats(
        ...
        try:
            for i in range(0, len(stats_data), batch_size):
                ...
                # Commit batch
                await session.commit()
                logger.info(f"Processed batch {i // batch_size + 1}: {len(batch)} stats records")
    ...
    async def sync_from_fpl_api(
        ...
        """
        logger.info(f"Starting ETL sync for season {season}, gameweek {gameweek or 'all'}")
        ...
        try:
            ...
            if gameweek:
                ...
                for player in players:
                    ...
                    if player_id:
                        try:
                            player_summary = await fpl_api_service.get_player_data(player_id)
                            history = fpl_api_service.extract_player_history(player_summary)
                            ...
                            logger.warning(f"Error fetching stats for player {player_id}: {str(e)}")
                            continue
            else:
                # All gameweeks
                for player in players:
