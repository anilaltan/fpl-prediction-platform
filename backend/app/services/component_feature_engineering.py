...
from app.models import PlayerGameweekStats, Player, EntityMapping
...
class ComponentFeatureEngineering:
    ...
    def extract_defcon_features(
        ...
        blocks_per_90 = (total_blocks / games_played) * (90.0 / max(total_minutes / games_played, 1.0))
        interventions_per_90 = (total_interventions / games_played) * (90.0 / max(total_minutes / games_played, 1.0))
        passes_per_90 = (total_passes / games_played) * (90.0 / max(total_minutes / games_played, 1.0))
        
        # Average per game
        avg_blocks = total_blocks / games_played if games_played > 0 else 0.0
        avg_interventions = total_interventions / games_played if games_played > 0 else 0.0
    ...
    def create_lag_features(
        ...
        for lag in lag_periods:
            if len(player_data) >= lag:
                xg_lag = player_data.iloc[lag - 1]['xg'] if lag <= len(player_data) else 0.0
                features[f'xg_lag_{lag}'] = float(xg_lag) if not pd.isna(xg_lag) else 0.0
            else:
        ...
        for window in [3, 5]:
            if len(player_data) >= window:
                ...
                features[f'xg_rolling_{window}'] = float(xg_rolling) if not pd.isna(xg_rolling) else 0.0
            else:
        ...
        for lag in lag_periods:
            if len(player_data) >= lag:
                xa_lag = player_data.iloc[lag - 1]['xa'] if lag <= len(player_data) else 0.0
                features[f'xa_lag_{lag}'] = float(xa_lag) if not pd.isna(xa_lag) else 0.0
            else:
        ...
        for window in [3, 5]:
            if len(player_data) >= window:
                ...
                features[f'xa_rolling_{window}'] = float(xa_rolling) if not pd.isna(xa_rolling) else 0.0
            else:
        ...
        for lag in lag_periods:
            if len(player_data) >= lag:
                cs_lag = player_data.iloc[lag - 1]['clean_sheets'] if lag <= len(player_data) else 0.0
                features[f'cs_lag_{lag}'] = float(cs_lag) if not pd.isna(cs_lag) else 0.0
            else:
        ...
        for window in [3, 5]:
            if len(player_data) >= window:
                ...
                features[f'cs_rolling_{window}'] = float(cs_rolling) if not pd.isna(cs_rolling) else 0.0
            else:
        ...
        if len(player_data) > 0:
            ...
            if len(games_with_minutes) > 0:
                cs_rate = (games_with_minutes['clean_sheets'] > 0).sum() / len(games_with_minutes)
    ...
    def prepare_xmins_features(
        ...
        for idx, row in training_data_sorted.iterrows():
            ...
            if fpl_id not in entity_mappings:
                logger.debug(f"No entity mapping for fpl_id={fpl_id}, continuing anyway")
        ...
        if null_counts.any():
            logger.warning(f"Found null values in xMins features: {null_counts[null_counts > 0].to_dict()}")
        ...
        logger.info(f"Prepared {len(features_df)} xMins feature rows with {len(features_df.columns)} features")
    ...
    def prepare_attack_features(
        ...
        for idx, row in training_data_sorted.iterrows():
            ...
            if len(player_historical) > 0:
                ...
                if total_minutes > 0:
                    ...
                    goals_per_90 = (player_historical['goals'].sum() / total_minutes) * 90.0
                    assists_per_90 = (player_historical['assists'].sum() / total_minutes) * 90.0
                else:
                    xg_per_90 = 0.0
                    xa_per_90 = 0.0
                    goals_per_90 = 0.0
            ...
            xg_labels.append(float(row.get('xg', 0.0)) if not pd.isna(row.get('xg')) else 0.0)
            xa_labels.append(float(row.get('xa', 0.0)) if not pd.isna(row.get('xa')) else 0.0)
        ...
        null_counts = features_df.isnull().sum()
        if null_counts.any():
            logger.warning(f"Found null values in Attack features: {null_counts[null_counts > 0].to_dict()}")
            features_df = features_df.fillna(0.0)
        ...
        logger.info(f"Prepared {len(features_df)} Attack feature rows with {len(features_df.columns)} features")
    ...
    def prepare_defense_features(
        ...
        for idx, row in training_data_sorted.iterrows():
            ...
            if len(player_historical) > 0:
                ...
                if total_minutes > 0:
                    ...
                    goals_conceded_per_90 = (player_historical['goals_conceded'].sum() / total_minutes) * 90.0
                else:
                    xgc_per_90 = 0.0
        ...
        null_counts = features_df.isnull().sum()
        if null_counts.any():
            logger.warning(f"Found null values in Defense features: {null_counts[null_counts > 0].to_dict()}")
            features_df = features_df.fillna(0.0)
        ...
        logger.info(f"Prepared {len(features_df)} Defense feature rows with {len(features_df.columns)} features")
    ...
    def verify_feature_distributions(
        ...
        for col in features_df.columns:
            ...
            if null_count > 0:
                logger.warning(f"{feature_type} feature '{col}' has {null_count} null values")
        ...
        logger.info(f"Feature distribution verification for {feature_type}: {len(stats)} features checked")
