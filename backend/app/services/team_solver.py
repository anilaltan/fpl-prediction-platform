...
class TeamSolver:
    ...
    def __init__(
        self, budget: float = 100.0, horizon_weeks: int = 3, free_transfers: int = 1
    ):
        """
    ...
    def solve(
        ...
        try:
            ...
            logger.info(
                f"Optimization completed. Total expected points: {solution['total_points']:.2f}"
            )
    ...
    def _extract_solution(
        ...
        for w in weeks:
            # Squad for week w
            squad = [i for i in player_ids if pulp.value(variables["x"][(i, w)]) == 1]
            squads[w] = squad

            # Starting XI for week w
            xi = [i for i in player_ids if pulp.value(variables["y"][(i, w)]) == 1]
            starting_xis[w] = xi

            # Transfers for week w
            transfers_in = [
                i for i in player_ids if pulp.value(variables["t"][(i, w)]) == 1
            ]
            transfers_out = [
                i for i in player_ids if pulp.value(variables["u"][(i, w)]) == 1
            ]
            ...
            transfers[w] = {
                ...
                "cost": max(
                    0,
                    (len(transfers_in) + len(transfers_out) - self.free_transfers)
    ...
    def calculate_expected_points(
        self, player: Dict, week: int, predictions: Optional[Dict] = None
    ) -> float:
        """
