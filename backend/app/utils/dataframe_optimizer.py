...
def optimize_dataframe_types(
    ...
    reduction_pct = ((original_memory - new_memory) / original_memory * 100) if original_memory > 0 else 0
    
    if reduction_pct > 5:  # Only log if significant reduction
        logger.debug(f"DataFrame memory optimized: {original_memory / 1024**2:.2f} MB -> {new_memory / 1024**2:.2f} MB ({reduction_pct:.1f}% reduction)")
...
def optimize_numeric_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        Optimized DataFrame
    """
    return optimize_dataframe_types(
        df,
...
def optimize_categorical_columns(df: pd.DataFrame, columns: Optional[List[str]] = None) -> pd.DataFrame:
    """
