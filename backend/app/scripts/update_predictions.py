...
async def update_predictions_for_gameweek(
    ...
    """
    try:
        logger.info(f"[BATCH PREDICTION] Starting prediction update for GW{gameweek} (season: {season})")
        
        # Initialize ML engine
        ml_engine = PLEngine()
        ml_engine._ensure_models_loaded()
        
        if not ml_engine.xmins_model.is_trained or not ml_engine.attack_model.xg_trained:
            logger.warning("[BATCH PREDICTION] ML models not trained, skipping prediction update")
            return {
                "status": "skipped",
        ...
        try:
            ...
            logger.warning(f"[BATCH PREDICTION] Failed to fetch ownership/availability data: {str(e)}")
        
        # Batch load all stats
        all_stats = db.query(PlayerGameweekStats).filter(
            ...
            try:
                ...
                if latest_stat:
                    ...
                    player_data.update({
                        ...
                        'xg_per_90': float(latest_stat.xg or 0) * (90.0 / mins_divisor),
                        'xa_per_90': float(latest_stat.xa or 0) * (90.0 / mins_divisor),
                        'form': player_form,
                        'ict_index': float(latest_stat.ict_index or 0)
                    })
                    
                    # Get recent stats (up to 5 gameweeks before current)
                    recent_stats = [s for s in player_stats if s.gameweek and s.gameweek < gameweek][:5]
                    if recent_stats:
                        player_data['recent_minutes'] = [s.minutes or 0 for s in recent_stats]
                        player_data['recent_xg'] = [float(s.xg or 0) for s in recent_stats]
                        player_data['recent_xa'] = [float(s.xa or 0) for s in recent_stats]
                ...
                if chance_of_playing is not None:
                    ...
                    if player_status in ['d', 'i', 'n', 's', 'u']:  # doubtful, injured, not available, suspended, unavailable
                        should_filter = True
                ...
                if updated_count % 100 == 0:
                    logger.info(f"[BATCH PREDICTION] Progress: {updated_count}/{len(players)} players processed")
        ...
        try:
            ...
            if db.dirty or db.new or db.deleted:
                logger.warning(f"[BATCH PREDICTION] Found pending changes at end, committing...")
                db.commit()
        except Exception as e:
...
async def update_predictions_for_current_gameweek():
    ...
    try:
        ...
        try:
            ...
            if not current_gw:
                logger.warning("[BATCH PREDICTION] Could not determine current gameweek")
                return
        ...
        logger.error(f"[BATCH PREDICTION] Error in update_predictions_for_current_gameweek: {str(e)}")
